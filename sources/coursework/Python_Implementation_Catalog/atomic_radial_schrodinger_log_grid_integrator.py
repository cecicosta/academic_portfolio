...
if __name__ == '__main__':
    ...
    # The following code uses a simple trapedio approximation to integrate the 
    # radial function along the radius from 0 to the practical infinity.
    r,P,E =radiallog(0, 1, 1,False)
    PIntegral = 0
    for i in range(1, len(P)):
        PIntegral = PIntegral + (r[i] - r[i-1]) * (P[i] + P[i-1])/2
    
    print("Integration of the radial function up to infinity: %f" %PIntegral)
    
    result={}
    for n in [2,3,4,5,7,9]:
        r,P,E=radiallog(1,n,1,False)
        plt.subplot(211)
#        plt.xlim(0, 250) 
        label = format("%dp" %(n))
        plt.plot(r, np.sqrt(r) * P, label=label)
        plt.xlabel("r (a.u.)")
        plt.ylabel("P")
        plt.legend(loc="upper center", ncols=6, fontsize=8)

        rr = np.sqrt(r)
        plt.subplot(212)
        plt.plot(rr, np.sqrt(r) * P, label=label)
        plt.xlabel('$\sqrt{\mathrm{r}}$ (a.u.)')
        plt.ylabel("P")        
#        plt.xlim(0, 18)
        plt.legend(loc="upper center", ncols=6, fontsize=8)
